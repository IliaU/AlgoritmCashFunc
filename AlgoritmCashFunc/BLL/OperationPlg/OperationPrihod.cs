...
namespace AlgoritmCashFunc.BLL.OperationPlg
{
    ...
    /// </summary>
    public sealed class OperationPrihod:Operation
    {
        /// <summary>
        /// ОКУД
        /// </summary>
        ...
        public OperationPrihod(int? Id, string OperationName, int KoefDebitor, int KoefCreditor) : base("OperationPrihod",  OperationName, KoefDebitor, KoefCreditor)
        {
            try
            {
                base.Id = Id;
                ...
                if (base.Id != null)
                {
                    OperationPrihod MyObj = this;
                    bool tt = Com.ProviderFarm.CurrentPrv.GetOperationPrihod(ref MyObj);
                }
            }
            catch (Exception ex)
            {
        ...
        /// Конструктор
        /// </summary>
        public OperationPrihod() : this(null, "Приходный ордер", 1, 0)
        {
            try
            {
            }
        ...
        protected override void SaveChildron()
        {
            try
            {
                if (Com.ProviderFarm.CurrentPrv.HashOperationPrihod(this))
                {
                    Com.ProviderFarm.CurrentPrv.UpdateOperationPrihod(this);
                }
                else  // Если нет то вставляем
                {
                    Com.ProviderFarm.CurrentPrv.SetOperationPrihod(this);
                }
            }
            catch (Exception ex)
            {
